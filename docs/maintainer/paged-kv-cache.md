...
sm_89 的 rotated family（`rk8v4` / `rk4v4` / `rk4v4-e8`）在编码前对每个 64-dim group 施加 H64
rotation；scale plane 与 INT8-G64 相同（FP16，\(X=D/64\)），code plane 为 int8（`rk8v4` 的 K）或
packed U8 \(X=D/2\)（int4，两码一字节）。donor（4090 fork）的 release notes 以每 token/head 的
半数记账，把这两行分别写作 196 B 和 132/136 B；本表给出的是实际 plane 字节数。

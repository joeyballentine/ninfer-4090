...
// so any grid is correct; this caps the launch when the work list is long. The per-SM factor is
// the design bound and the SM count comes from the live device, so the cap tracks the part.

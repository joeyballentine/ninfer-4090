...
KvCacheStorage parse_kv_cache(std::string_view text) {
    ...
    // sm_89 rotated family; the spellings match the serve CLI (serve_options.cpp).
    ...
    throw std::invalid_argument("--kv-dtype must be bf16, int8, fp8, nvfp4, k8v4, rk8v4, rk4v4, "
                                "rk4v4-e8, or rk2v4-e8");
}
...
std::string usage_text(std::string_view program) {
    ...
    out << "Usage: " << program << " --weights <artifact.ninfer> [options]\n\n"
        ...
        << "  --draft-tokens <n>         MTP 1..15; DFlash/DFlash2 1..15\n"

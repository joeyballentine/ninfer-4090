...
// Qualification for the sm_89 FP8 prefill route of Q5_G64_FP16 Linear.
//
...
// was written for:
//
//   * Weight side: exact. A Q5 code is a 5-bit two's-complement integer in [-16,15]; E4M3 carries
...
int check_workspace_contract(std::int32_t n, std::int32_t k) {
    ...
    const auto capacity = [&](ops::LinearPolicy policy, std::int32_t t) {
        return ops::linear_workspace_capacity_bytes(QType::Q5_G64_FP16, n, k, policy, t, t);
    };
    ...
    if (reported < payload_2048 || reported > payload_2048 + 512U ||
        ...
        std::cerr << "Q5 A8 prefill workspace contract mismatch for N=" << n << " K=" << k << '\n';
        ++failures;
    }
    // An interval is sized by its top extent, and a purely sub-threshold interval needs nothing.
    if (ops::linear_workspace_capacity_bytes(QType::Q5_G64_FP16, n, k, ops::LinearPolicy::AllowA8,
                                             1, 2048) != reported ||
        ops::linear_workspace_capacity_bytes(QType::Q5_G64_FP16, n, k, ops::LinearPolicy::AllowA8,
                                             1, 127) != 0) {
        std::cerr << "Q5 A8 prefill workspace interval mismatch for N=" << n << " K=" << k << '\n';
        ++failures;
    }
    return failures;
}

int run_q5_a8_prefill() {
    ...
    int failures = 0;
    for (const auto shape : {std::pair<std::int32_t, std::int32_t>{5120, 17408},
                             std::pair<std::int32_t, std::int32_t>{6144, 5120}}) {
        const std::uint32_t seed = 977U + static_cast<std::uint32_t>(shape.first % 1000);
        failures += run_shape("Q5_A8_PREFILL", ActivationCompute::A8, make_q5_g64_fp16_weight,
        ...
        failures += run_shape("Q5_A8_PREFILL_BELOW", ActivationCompute::A16,
...
int main() {
    ...
    std::cout << "SKIP: Q5 A8 prefill is an sm_89 build route\n";
    return 77;
#else
    ...
    if (!ada_device()) {
        std::cout << "SKIP: Q5 A8 prefill requires an sm_89 device\n";
        return 77;
    }
    try {
        ninfer::ops::set_prefill_a8_routes_enabled(true);
        const int failures = run_q5_a8_prefill();
        ninfer::ops::set_prefill_a8_routes_enabled(false);
        std::cout << (failures == 0 ? "OK" : "FAIL") << " Q5 A8 prefill Linear\n";
        return failures == 0 ? 0 : 1;
    } catch (const std::exception& error) {
        std::cerr << "Q5 A8 prefill Linear: " << error.what() << '\n';
        return 1;
    }
#endif
}
